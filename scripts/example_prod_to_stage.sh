...
# Arguments
stage_host=${1-devops@stage.civictechindex.org}
prod_host=${2-devops@civictechindex.org}

echo "Creating database dump from stage..."
ssh $prod_host "pg_dump -h localhost -Fc -f /tmp/db-dump.sql -U postgres civictechindex -x -O"
...
ssh $stage_host "sudo -u postgres pg_restore --clean -h localhost -d civictechindex -U postgres '/tmp/db-dump.sql'"
...
ssh $stage_host "$manage_prefix python manage.py change_site_domain --site_id=1 --new_site_domain='stage.civictechindex.org'"

echo "Syncing s3 buckets..."
aws --profile civictechindex_devops s3 sync s3://s3.stage.civictechindex.org s3://s3.civictechindex.org --acl public-read
