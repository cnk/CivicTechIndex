...
2. Include this ip on your hosts-file
    ...
    ```
    127.0.0.1 civictechindex.org.test
    ```

    On windows you can run this command to append it:

    ```
    echo 127.0.0.1 civictechindex.org.test >> c:\windows\System32\drivers\etc\hosts
    ```

3. Add root cert: `mkcert -install` (if not already available)
4. Generate ssl certs for local development:
    ```
    mkcert --cert-file docker/files/certs/cert.pem --key-file docker/files/certs/cert-key.pem civictechindex.org.test
    ```
5. Start project
...
6. Visit your site on: [https://civictechindex.org.test:8081](https://civictechindex.org.test:8081)
    - ...or login to [https://civictechindex.org.test:8081/wt/cms](https://civictechindex.org.test:8081/wt/cms) (Username: `admin` and password: `admin`)
...
CivicTechIndex is released under the MIT license.
